    ...
    try {
        ...
        await prisma.$transaction(async (transaction) => {
         ...
         FOR UPDATE SKIP LOCKED`;
            ...
            const recipientIds = messages.map((msg) => msg.recipient).filter(id => id !== null)

            let usersWithEmails = await prisma.users.findMany({
                where: {
                    id: {
                        in: recipientIds
                    }
            ...
            const userIdToEmailMap = usersWithEmails.reduce((prev, curr) => prev.set(curr.id, curr.email ?? undefined), new Map<string, string|undefined>())
