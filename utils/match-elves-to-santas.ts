 ...
 * */
export function matchElvesToSantas(elves: SecretSanta[]): SecretSanta[][] {
    const unMatchedElves = [...elves.map(elf => ({...elf, actsAsElfFrom: undefined, actsAsSantaTo: undefined}))] as SecretSanta[]
    ...
    while (unMatchedElves.length > 0) {
        ...
        const validSantas = unMatchedSantas.filter(santa => validateMatch(santa, nextElf))
        ...
        while (validSantas.length > 0 && !assignedSanta) {
            ...
            if (!failedSequences.some(seq => sequencesAreEqual(seq, resultingSequence))) {
