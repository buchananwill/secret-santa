...
export function checkForCycle(santa: SecretSanta, limit: number): SecretSanta[] {
...
export function sequencesAreEqual(santaSequence: SecretSanta[], otherSequence: SecretSanta[]) {
    ...
    for (let i = 0; i < santaSequence.length; i++) {
        if (santaSequence[i] !== otherSequence[i]) {
            // console.log('different sequence')
            return false;
        }
    }
